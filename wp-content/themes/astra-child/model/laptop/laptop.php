...
function device_crud_laptop()
{
    ...
    $all_brands = $wpdb->get_col("SELECT DISTINCT Brand FROM $table_device_wn WHERE Category = 'Laptop' AND Brand != '' ORDER BY Brand");
    $all_departments = $wpdb->get_col("SELECT DISTINCT Department FROM $table_device_wn WHERE Category = 'Laptop' AND Department != '' ORDER BY Department");
    // -------------------------------------------
    $total_items = $wpdb->get_var("SELECT COUNT(*) FROM $table_device_wn $where_sql");
    $total_pages = ceil($total_items / $page);

    $rows = $wpdb->get_results("SELECT * FROM $table_device_wn $where_sql 
    ...
    <div class="container-fluid" style="margin: 0 -40px;">
        ...
        function toggleDepartment() {
            ...
            if (status === 'Available') {
                ...
                deptSelect.value = ''; // clear department when hiding
            } else {
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="importCsvModalLabel">📥 Import Laptops (CSV)</h5>
        ...
        <form method="POST" action="" id="bulk-action-form-laptop">
            <div class="d-flex align-items-center justify-content-between mb-3">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="<?= esc_url(add_query_arg(['export_csv' => 'device', 'category' => 'Laptop'], $_SERVER['REQUEST_URI'])) ?>" class="btn btn-secondary btn-sm">
                        📤 Export CSV
                    </a>
                </div>
            </div>
        ...
        <div class="table-responsive-xl rounded">
            <table class="table table-bordered table-sm text-center">
                <thead class="table-secondary">
                    <tr>
                        <th class="text-nowrap py-3" style="width: 40px;"><input type="checkbox" id="selectAll-laptop"></th>
                ...
                <tbody>
                    <?php foreach ($rows as $row): ?>
                        <tr class="text-nowrap py-2" style="white-space: nowrap;">
                            <td class="align-middle"><input type="checkbox" name="bulk_device_ids[]" value="<?= $row->DeviceID ?>" class="device-checkbox-laptop" data-sn="<?= esc_attr($row->SerialNumber ?? '') ?>"></td>
