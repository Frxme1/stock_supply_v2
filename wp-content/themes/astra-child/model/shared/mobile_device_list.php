    ...
    <?php foreach ($rows as $row): ?>
        <div class="mobile-device-card">
            <div class="mobile-device-header">
                ...
                if (strcasecmp($row->Status, 'Available') === 0) $statusClass = 'status-available';
                elseif (strcasecmp($row->Status, 'In Use') === 0) $statusClass = 'status-inuse';
                elseif (strcasecmp($row->Status, 'Maintenance') === 0) $statusClass = 'status-maintenance';
                ?>
            ...
            <div class="mobile-device-body">
                <div class="mobile-device-owner">
                    ...
                    if ($owner === '' && $nickname === '') {
                        ...
                        if ($nickname !== '') echo htmlspecialchars($nickname) . ' ';
                        if ($owner !== '') {
            ...
            <div class="mobile-device-actions">
                <?php if (strcasecmp($row->Status, 'Maintenance') === 0): ?>
                    <a href="?maintenance=<?= $row->DeviceID ?>" class="mobile-btn-action mobile-btn-secondary"><i class="fa-solid fa-gear"></i> Edit</a>
                <?php else: ?>
                    <a href="?edit=<?= $row->DeviceID ?>" class="mobile-btn-action mobile-btn-secondary"><i class="fa-solid fa-gear"></i> Edit</a>
                <?php endif; ?>
                <a href="?view=<?= esc_attr($row->DeviceID) ?>" class="mobile-btn-action mobile-btn-primary"><i class="fa-solid fa-magnifying-glass"></i> View</a>
            </div>
        </div>
    <?php endforeach; ?>
</div>
